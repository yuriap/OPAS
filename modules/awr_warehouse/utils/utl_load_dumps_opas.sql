...
begin
   p(l_header_scr);
   for i in (SELECT PROJ_ID, PROJ_NAME, PROJ_NOTE, CREATED, owner, KEEP_FOREVER, IS_PUBLIC FROM AWRWH_PROJECTS order by PROJ_ID) loop
     p(replace(replace(replace(replace(replace(replace(l_proj_scr,'<Project name>',i.PROJ_NAME)
                                                                 ,'<Project description>',i.PROJ_NOTE)
                                                                 ,'<PROJ_DATE>',to_char(i.CREATED,'YYYYMMDDHH24MISS'))
                                                                 ,'<OWNER>',i.owner)
                                                                 ,'<KEEPFOREVER>',i.KEEP_FOREVER)
                                                                 ,'<ISPUBLIC>',i.IS_PUBLIC));
     p('--');
     for j in (SELECT FILENAME, 
                      DUMP_DESCRIPTION,LOADED,
                      dbid,
                      min_snap_id,
                      max_snap_id,
                      min_snap_dt,
                      max_snap_dt,
                      db_description, 
                      dump_name FROM AWRWH_DUMPS where PROJ_ID=i.PROJ_ID order by dump_id) loop
       ...
       p(replace(
         ...
         replace(l_dump_scr,'<dump_file_name>',j.FILENAME),
                            ...
                            '<DUMP_DATE>',to_char(j.LOADED,'YYYYMMDDHH24MISS')),
