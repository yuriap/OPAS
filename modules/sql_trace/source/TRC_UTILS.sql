create or replace PACKAGE TRC_UTILS AS
  ...
  procedure set_project_retention(p_retention   trc_projects.retention%type,
                                  p_trcproj_id  trc_projects.trcproj_id%type);
  ...
  procedure store_file_from_view(p_trc_file_id TRC_FILE.trc_file_id%type);
...
create or replace PACKAGE BODY TRC_UTILS AS
  ...
  begin
    ...
    if p_retention not in ('DEFAULT','KEEPFOREVER','KEEPFILESONLY','KEEPPARSEDONLY','CLEANUPOLD') then raise_application_error(-20000,'Invalid retention specified'); end if;
  ...
  begin
    ...
    INSERT INTO trc_file_source ( trc_file_id, file_db_source, file_content ) VALUES (p_trc_file_id, p_db_source, p_file_content);
  end;
  ...
  begin
    if p_trc_file_id is null then --project note
      ...
      raise_application_error(-20000,'Invalid inout for set_note.');
    end if;
  end;
