create or replace package TRC_PROJ_LCC as
  ...
  procedure project_exec_action(p_proj trc_projects%rowtype, p_action number);

  --Project action availability
  function project_check_action(p_proj_id trc_projects.proj_id%type, p_action number) return boolean;
  function project_check_action(p_proj trc_projects%rowtype, p_action number) return boolean;
